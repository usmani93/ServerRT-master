...
namespace ServerRT.RTHub
{
    public class RTHub : Hub<IConnectionHub>
    {
        private readonly List<User> _Users;
        private readonly List<UserCall> _UserCalls;
        private readonly List<CallOffer> _CallOffers;

        public RTHub(List<User> users, List<UserCall> userCalls, List<CallOffer> callOffers)
        {
            _Users = users;
        ...
        /// <summary>
        /// Client-requested disconnect: hang up any calls and abort the connection.
        /// Calling this will trigger OnDisconnectedAsync server-side cleanup via Context.Abort().
        /// </summary>
        public async Task Disconnect()
        {
            // Make sure any active calls are terminated and other parties are notified
            await HangUp();

            // Abort the connection which will invoke OnDisconnectedAsync and perform cleanup
            Context.Abort();
        }
